; TraeCode CN Enhancer - Windows 安装包配置
;
...
;   - 不禁用目录页，用户可自选安装位置；
;   - 自定义页紧跟在目录页之后，让用户确认 Trae CN 主程序位置；
;   - 检测在 Pascal 内用注册表与文件存在性完成，不解析子进程输出：
;     子进程 stdout 是 UTF-8，而安装程序按系统 ANSI 解码，中文路径会损坏；
;   - 保存用户选择时把路径作为**命令行参数**传给 TraeCodeEnhancer.exe configure，
;     命令行是 Unicode，不存在解码问题。
  ...
  #define OutputBaseFilename "TraeCodeEnhancer-Setup"
#endif

#define ProductName "TraeCode CN Enhancer"
#define ProductExe "TraeCodeEnhancer.exe"

[Setup]
AppId={{BB144F88-F2C9-4DBC-AE6B-BD489F0D0A35}
AppName={#ProductName}
...
Name: "{group}\{#ProductName}"; Filename: "{sys}\wscript.exe"; Parameters: """{app}\scripts\launch-hidden.vbs"""; WorkingDir: "{app}"; Comment: "启动 Trae CN 与增强面板"
...
[Run]
Filename: "{sys}\wscript.exe"; Parameters: """{app}\scripts\launch-hidden.vbs"""; WorkingDir: "{app}"; Description: "立即启动 Trae CN 与增强面板"; Flags: nowait postinstall skipifsilent
...
[Code]
const
  TraeExeName = 'Trae CN.exe';
  ProductMarkerCode = 'TRAECODE CN';
  ProductMarkerGeneric = 'TRAE CN';
...
begin
  Result := '';
  if RegQueryStringValue(HKCU, 'Software\TraeCodeCnEnhancer', 'TraeExe', Value) then
    Result := Value;
end;

function SaveTraePath(const Path: String): Boolean;
begin
  Result := RegWriteStringValue(HKCU, 'Software\TraeCodeCnEnhancer', 'TraeExe', Path);
end;
...
{ 只接受 DisplayName 明确含 TraeCode CN 或 Trae CN 的项；宽松匹配 "TRAE" 会命中
  无关产品 TraeWork（TRAE SOLO CN workspace），那会导致后续所有重启 TRAE 的流程失败。 }
...
begin
  ...
  Uppered := Uppercase(DisplayName);
  if (Pos(ProductMarkerCode, Uppered) = 0) and (Pos(ProductMarkerGeneric, Uppered) = 0) then Exit;
...
begin
  Result := '';
  Candidates[0] := ExpandConstant('{localappdata}\Programs\Trae CN\') + TraeExeName;
  Candidates[1] := ExpandConstant('{localappdata}\Trae CN\') + TraeExeName;
  Candidates[2] := ExpandConstant('{localappdata}\Programs\') + TraeExeName;
  Candidates[3] := ExpandConstant('{commonpf}\Trae CN\') + TraeExeName;
  Candidates[4] := ExpandConstant('{commonpf}\') + TraeExeName;
  Candidates[5] := ExpandConstant('{commonpf32}\Trae CN\') + TraeExeName;
  Candidates[6] := ExpandConstant('{commonpf32}\') + TraeExeName;
  Candidates[7] := ExpandConstant('{userappdata}\Trae CN\') + TraeExeName;
  for Index := 0 to 7 do
  begin
...
begin
  if Path = '' then
    Result := '未自动找到 Trae CN，请点击“浏览”手动选择主程序。'
  else
    Result := '已自动检测到 Trae CN，可直接继续；如果不对请点击“浏览”修改。';
end;
...
begin
  ...
  if Candidate = '' then
    ...
    TraeStatusLabel.Caption := '文件名必须是 Trae CN.exe，请重新选择。'
  else
...
begin
  ...
  if not Result then
    MsgBox('请选择名为 Trae CN.exe 的主程序。', mbError, MB_OK);
end;

procedure InitializeWizard();
begin
  // 已安装时先让用户决定是重新安装还是退出，避免覆盖时误操作。
  // 不能用 ExpandConstant 的 app 常量：{{app}} 在 InitializeWizard 阶段尚未初始化会直接抛错。
...
begin
  ...
  begin
    Result := '无法停止正在运行的 TraeCode CN Enhancer 服务，请先手动停止后重试。';
    Exit;
  end;
...
begin
  ...
  begin
    if WizardSilent then
      Log('Trae CN.exe was not selected; configure it later with configure --trae-exe')
    else
      MsgBox('没有可保存的 Trae CN 路径，请稍后运行 configure --trae-exe。', mbInformation, MB_OK);
    Exit;
  end;
  ...
  begin
    if WizardSilent then
      Log('unable to launch TraeCodeEnhancer configure')
    else
      MsgBox('写入 TRAE 路径失败，可稍后手动运行：' + #13#10 +
        'TraeCodeEnhancer.exe configure --trae-exe "<路径>"', mbInformation, MB_OK);
  end
  else if ResultCode <> 0 then
  begin
    if WizardSilent then
      Log('TraeCodeEnhancer configure exited with code ' + IntToStr(ResultCode))
    else
      MsgBox('TraeCodeEnhancer 未能保存 TRAE 路径（退出码 ' + IntToStr(ResultCode) + '）。', mbInformation, MB_OK);
  end;
