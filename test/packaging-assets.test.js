...
test("the packaged launcher hides its console and stays pure ASCII", () => {
  ...
  const vbs = buildLaunchVbs({
    nodePath: path.join(PROJECT_ROOT, "dist", "portable", "TraeCodeEnhancer.exe"),
  });
...
test("the packaged launcher reports a missing executable instead of doing nothing", () => {
  ...
  assert.ok(vbs.includes(", 48, \"TraeCode CN Enhancer\""), "the dialog needs a title and warning icon");
...
test("the cmd entry supports both source-build and portable layouts", async () => {
  ...
  assert.match(text, /dist\\TraeCodeEnhancer\.exe/);
  assert.match(text, /if exist "TraeCodeEnhancer\.exe"/);
