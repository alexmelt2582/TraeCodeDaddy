...
test("verification URI matches Trae CN authorization parameters", () => {
  const uri = new URL(
    buildVerificationUri(
      context,
  ...
  // Trae CN is a full IDE: the real client emits auth_from="trae" (only
  // SOLO_Lite uses "solo"), and hide_saas_login is added only for solo.
  assert.equal(uri.searchParams.get("auth_from"), "trae");
  assert.equal(uri.searchParams.get("client_id"), "ono9krqynydwx5");
  ...
  assert.equal(uri.searchParams.get("hide_saas_login"), null);
});
