...
test("process ownership requires both a managed image and the expected command line", () => {
  ...
  assert.equal(
    isManagedProcessCommand(
      "D:\\app\\TraeCodeEnhancer.exe --internal-daemon",
      "daemon",
      { bundled: true },
    ),
    true,
  ...
  assert.equal(
    isManagedProcessCommand("D:\\app\\TraeCodeEnhancer.exe --internal-watchdog", "daemon", {
      bundled: true,
    }),
    false,
  );
...
test("only this project's source and bundled process images are managed", () => {
  ...
  assert.equal(isManagedProcessImage("TraeCodeEnhancer"), true);
  assert.equal(isManagedProcessImage("trae-code-cn-enhancer"), true);
  assert.equal(isManagedProcessImage("powershell"), true);
  assert.equal(isManagedProcessImage("Trae CN"), false);
