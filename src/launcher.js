...
const cdpPort = parsePort(
  argumentValue("--cdp-port") || process.env.TRAECODE_ENHANCER_CDP_PORT,
  DEFAULT_CDP_PORT,
);
const uiPort = parsePort(
  argumentValue("--ui-port") || process.env.TRAECODE_ENHANCER_UI_PORT,
  DEFAULT_UI_PORT,
);
const explicitTraeExe = argumentValue("--trae-exe");
let exePath = explicitTraeExe || process.env.TRAECODE_ENHANCER_TRAE_EXE || DEFAULT_TRAE_EXE;
const userDataDir =
  argumentValue("--user-data-dir") ||
  process.env.TRAECODE_ENHANCER_USER_DATA_DIR ||
  DEFAULT_TRAE_USER_DATA_DIR;
const dataDir =
  argumentValue("--data-dir") || process.env.TRAECODE_ENHANCER_DATA_DIR || DEFAULT_DATA_DIR;
const storagePath =
  process.env.TRAECODE_ENHANCER_STORAGE_PATH ||
...
async function startDaemon() {
  ...
  const child = spawn(launch.command, launch.args, {
    ...
    env: {
      ...
      TRAECODE_ENHANCER_CDP_PORT: String(cdpPort),
      TRAECODE_ENHANCER_UI_PORT: String(uiPort),
      TRAECODE_ENHANCER_DATA_DIR: dataDir,
      TRAECODE_ENHANCER_STORAGE_PATH: storagePath,
      TRAECODE_ENHANCER_USER_DATA_DIR: userDataDir,
    },
  });
  child.unref();
...
async function main() {
  ...
  // A live CDP port alone proves nothing: the instance holding it may be running
  // under another enhancer's env with a different user-data-dir, and injecting
  // into it produces a panel whose API_BASE/storage disagree with this daemon
  // (the "Failed to fetch" loop). Only reuse an instance we own.
  if (cdpReady && !noRestart) {
    const owner = await isTraeCdpManagedBy(cdpPort, userDataDir);
    if (owner === "foreign") {
      console.warn(
        `[${APP_NAME}] CDP ${cdpPort} is held by a foreign Trae CN profile (different --user-data-dir); restarting and taking it over...`,
      );
      cdpReady = false;
    } else if (owner === "none") {
      // Port answered but no matching process is visible; do not restart blindly,
      // but warn so the mismatch is not silent.
      console.warn(
        `[${APP_NAME}] CDP ${cdpPort} answered but no owned Trae CN process matched; proceeding with reuse (might be a cross-app occupant).`,
      );
    }
  }
  ...
  if (!cdpReady && noRestart) {
    throw new Error(
      `Trae CN is not exposing CDP port ${cdpPort}. Start it through this launcher.`,
    );
  }

  if (!cdpReady) {
    console.log(`[${APP_NAME}] Restarting Trae CN with CDP port ${cdpPort}...`);
    await stopTraeForRestart(exePath);
    await startTraeWithCdp(exePath, cdpPort, userDataDir);
    cdpReady = await waitForCdp(cdpPort);
    if (!cdpReady) {
      throw new Error(`Timed out waiting for Trae CN CDP port ${cdpPort}`);
    }
  }
