/**
 * Locates the Trae CN executable.
 *
 ...
 *   3. the `TRAECODE_ENHANCER_TRAE_EXE` environment variable
...
export const TRAE_EXE_NAME = "Trae CN.exe";
export const TRAE_PROCESS_NAME = "Trae CN";

export const SOURCES = {
  explicit: "命令行参数 --trae-exe",
  config: "配置文件 config.json",
  env: "环境变量 TRAECODE_ENHANCER_TRAE_EXE",
 ...
 * as `TRAE` also matches the unrelated "TRAE SOLO CN" workspace, and picking that executable
 * would break every flow that restarts TRAE.
 */
export function isProductDisplayName(displayName) {
  return typeof displayName === "string" && /Trae(Code)? CN/i.test(displayName);
}
...
async function* sourceValues({ explicit, configured, env, probe }) {
  ...
  yield { value: env.TRAECODE_ENHANCER_TRAE_EXE, source: "env" };
...
async function windowsSnapshot() {
  const script = [
    ...
    ")",
    "$uninstall = @(Get-ItemProperty $keys | Where-Object { $_.DisplayName -match 'Trae(Code)? CN' } | Select-Object DisplayName, InstallLocation, DisplayIcon)",
...
export function formatNotFoundHelp({ attempts, dataDir }) {
  ...
  lines.push('  TraeCodeEnhancer.exe configure --trae-exe "D:\\你的路径\\Trae CN.exe"');
