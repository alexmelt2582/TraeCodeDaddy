...
export const AUTOSTART_SHORTCUT_NAME = "TraeCode CN Enhancer";
...
export const LAUNCH_MISSING_EXE_MESSAGE =
  "TraeCode CN Enhancer cannot start: TraeCodeEnhancer.exe was not found next to this script. "
...
export function buildAutostartVbs({
  ...
  const guard = (condition, exitCode) => {
    ...
    lines.push(`  MsgBox ${vbsQuote(missingExeMessage)} & vbCrLf & vbCrLf & nodeExe, 48, "TraeCode CN Enhancer"`);
...
export function buildShortcutScript({
  ...
  vbsPath,
  workingDirectory,
  description = "TraeCode CN Enhancer background service",
}) {
