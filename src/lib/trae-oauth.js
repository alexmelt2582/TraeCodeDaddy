...
<head>
  ...
  <title>TraeCode CN Enhancer</title>
  <style>
...
<body>
  <main>
    ...
    <footer>TraeCode CN Enhancer 本地回调服务</footer>
  </main>
  ${redirect}
</body>
</html>`;
...
export function buildVerificationUri(context, callbackUrl, loginTraceId, codeChallenge, loginHost) {
  ...
  // Trae CN is a full IDE, not SOLO_Lite: the real client emits auth_from="trae"
  // (only SOLO_Lite uses "solo"). Sending "solo" while the exchange posts
  // PlatformCode IDE_PC makes api.trae.com.cn reject the code with 20403.
  appendQuery(parts, "auth_from", "trae", false);
  ...
  for (const origin of origins) {
    try {
      const response = await requestJson(`${origin}${GUIDANCE_PATH}`, {
        ...
        headers: { "user-agent": "TraeCode CN Enhancer/0.1" },
      });
      if (!response.ok) {
...
async function exchangeAuthCode(context, authCode, verifier, userTag, loginHost, logger) {
  ...
  for (const origin of origins) {
    ...
    try {
      ...
      const errCode = response.json && response.json.ResponseMetadata && response.json.ResponseMetadata.Error && response.json.ResponseMetadata.Error.Code ? response.json.ResponseMetadata.Error.Code : "";
      logger?.log(`[oauth-probe] exch ${origin} status=${response.status} code=${errCode} result=${response.json && response.json.Result !== undefined} keys=${response.json ? Object.keys(response.json).join(",") : "none"} bodyLen=${(response.text || "").length}`);
      if (!response.ok) {
        lastError = safeRemoteError(response);
        continue;
      }
...
export async function openExternal(url) {
  try {
    await execFileAsync(
      "powershell.exe",
      ["-NoProfile", "-Command", "Start-Process -FilePath $env:TRAECODE_ENHANCER_OPEN_URL"],
      {
        timeout: 10000,
        windowsHide: true,
        env: { ...process.env, TRAECODE_ENHANCER_OPEN_URL: url },
      },
    );
    return "powershell";
  } catch {
    try {
      await execFileAsync(
        "cmd.exe",
        ["/d", "/s", "/c", 'start "" "%TRAECODE_ENHANCER_OPEN_URL%"'],
        {
          timeout: 10000,
          windowsHide: true,
          env: { ...process.env, TRAECODE_ENHANCER_OPEN_URL: url },
...
export class TraeOAuthManager {
  ...
  async start() {
    ...
    // The real Trae CN authorization URL and the DeviceInfo in the exchange both
    // carry the same real-storage device_id and machine_id (getDeviceId()). Using
    // a random identity would split the device between auth and exchange and is
    // rejected with 20403. Keep the real identity for both, matching the client.
  ...
  async handleCallback(session, request, response) {
    ...
    sendHtml(
      ...
      callbackPage({
        tone: "success",
        title: "授权成功",
        message: "登录信息已传回 TraeCode CN Enhancer，可以关闭此页面。",
      }),
    );
    this.closeSessionServer(session);
