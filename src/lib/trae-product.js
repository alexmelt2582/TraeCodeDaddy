...
const DEFAULT_CLIENT_ID = "ono9krqynydwx5";
...
async function readProductInfo(exePath) {
  ...
  const authConfig = root.iCubeApp?.authConfig?.TRAE || {};
  return {
...
export async function collectLoginContext({ exePath, storageRoot }) {
  ...
  const deviceId = parseDeviceId(storageRoot) || crypto.randomInt(10 ** 15, 10 ** 15 + 2 ** 47).toString();
  const machineId =
