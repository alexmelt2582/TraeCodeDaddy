...
import { attachWebSocketJson } from "./lib/ws.js";
...
const CDP_PORT = parsePort(process.env.TRAECODE_ENHANCER_CDP_PORT, DEFAULT_CDP_PORT);
const UI_PORT = parsePort(process.env.TRAECODE_ENHANCER_UI_PORT, DEFAULT_UI_PORT);
const DATA_DIR = process.env.TRAECODE_ENHANCER_DATA_DIR || DEFAULT_DATA_DIR;
const STORAGE_PATH = process.env.TRAECODE_ENHANCER_STORAGE_PATH || DEFAULT_STORAGE_PATH;
/**
 ...
 */
let TRAE_EXE = process.env.TRAECODE_ENHANCER_TRAE_EXE || DEFAULT_TRAE_EXE;
...
const AUTO_KEEPALIVE_ENABLED = process.env.TRAECODE_ENHANCER_AUTO_KEEPALIVE !== "0";
const AUTO_KEEPALIVE_INTERVAL_MS = parseDuration(
  process.env.TRAECODE_ENHANCER_KEEPALIVE_INTERVAL_MS,
  DEFAULT_KEEPALIVE_INTERVAL_MS,
);
const AUTO_KEEPALIVE_RETRY_INTERVAL_MS = parseDuration(
  process.env.TRAECODE_ENHANCER_KEEPALIVE_RETRY_INTERVAL_MS,
  DEFAULT_KEEPALIVE_RETRY_INTERVAL_MS,
);
const AUTO_KEEPALIVE_SWEEP_INTERVAL_MS = parseDuration(
  process.env.TRAECODE_ENHANCER_KEEPALIVE_SWEEP_INTERVAL_MS,
  30 * 60 * 1000,
);
...
async function stopTraeForSwitch() {
  if (!(await traeExecutableExists(TRAE_EXE))) {
    throw new Error(
      [
        `Trae CN 的可执行文件不存在：${TRAE_EXE}`,
        "请先指定一次路径，之后会被记住：",
        '  TraeCodeEnhancer.exe configure --trae-exe "D:\\路径\\Trae CN.exe"',
      ].join("\n"),
    );
  }
  ...
  if ((await findTraeProcessIds(TRAE_EXE)).length) {
    throw new Error("Trae CN did not remain fully stopped");
  }
}
...
async function main() {
  ...
  if (traeResolution.path) {
    ...
    console.warn(
      "[trae] 未找到 Trae CN：账号切换与登录流程不可用；签到与保活不受影响。",
    );
    console.warn(
      '[trae] 请运行 configure --trae-exe "D:\\路径\\Trae CN.exe" 指定一次，之后会记住。',
    );
  }
  ...
  async function scheduleAutoCheckin({ initialRun = false } = {}) {
    ...
    if (process.env.TRAECODE_ENHANCER_AUTO_CHECKIN === "0") return;
  ...
  const oauthManager = new TraeOAuthManager({
    ...
    storagePath: STORAGE_PATH,
    exePath: TRAE_EXE,
    openBrowser: process.env.TRAECODE_ENHANCER_OPEN_BROWSER !== "0",
  ...
  // Trae CN's workbench CSP forbids `connect-src http:` but allows `ws:`, so the
  // injected panel reaches this daemon over WebSocket instead of fetch. Each JSON
  // message is turned into a synthetic HTTP request and routed through the same
  // `route()` logic the normal API uses, then the status/body are sent back.
  attachWebSocketJson(server, "/panel", (msg, reply) => {
    const body =
      msg.body === undefined || msg.body === null
        ? undefined
    ...
    const request = {
      ...
      headers:
        msg && msg.token
          ? { "x-trae-enhancer-token": String(msg.token) }
          : {},
    ...
    const response = {
      writeHead(status) {
        ...
        reply({
          ...
          status: this.status,
          data,
          error: this.status >= 400 ? text : undefined,
        });
      },
    };
    route(request, response, apiToken, cdpClient, oauthManager, fakeLogoutManager)
      .catch(async (error) => {
        if (!responded) {
          responded = true;
          reply({
