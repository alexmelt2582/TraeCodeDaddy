(() => {
  ...
  // A failure against this base is the symptom that separates "this window runs
  // under another enhancer's env/port" from every other state. Logging it here
  // (not the daemon's log) is deliberate: when the fetch itself fails the panel
  // can't reach the daemon at all, so the page console is the only surviving
  // evidence until a daemon that owns this window injects again.
  console.log(`[trae-enhancer] panel booted with apiBase=${API_BASE} appVersion=${APP_VERSION}`);
  ...
  aboutPane.innerHTML = `
    <div class="te-help">
      ...
      <footer class="te-help-version">TraeCode CN Enhancer v${APP_VERSION}</footer>
    </div>
  `;
  ...
  function panicRpc(error) {
    const entries = [...rpcPending.values()];
    rpcPending.clear();
  ...
  function openRpcSocket() {
    return new Promise((resolve, reject) => {
      let ws;
      try {
        ws = new WebSocket(`${API_BASE.replace(/^http:/, "ws:")}/panel`);
      } catch (error) {
        rpcSocket = null;
        panicRpc(new Error(`无法连接增强服务 (${API_BASE})`));
        reject(error);
        return;
      }
      ws.onopen = () => {
      ...
      ws.onmessage = (event) => {
        ...
        else entry.reject(new Error(msg.error || `HTTP ${msg.status}`));
      };
    });
  }

  function sendRpc(msg) {
    if (rpcSocket && rpcSocket.readyState === 1) {
      rpcSocket.send(JSON.stringify(msg));
      return;
    }
    if (!rpcOpen) {
      rpcOpen = openRpcSocket()
        .catch(() => undefined)
        .finally(() => {
          rpcOpen = null;
        });
  ...
  // Trae CN's workbench CSP blocks `connect-src http:`, so the panel talks to the
  // daemon over the `ws:` channel it explicitly allows. The HTTP API surface is
  // unchanged; only the transport below differs from the old `fetch`.
  function api(path, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`增强服务请求超时: ${path}`));
      }, 20000);
  ...
  async function reconcileWithDaemon() {
    ...
    try {
      ...
      // Never swallow a panel-to-daemon failure silently: it is the one symptom
      // that separates "this window runs under another enhancer's env/port" from
      // every other state, and a bare "Failed to fetch" explains nothing.
      console.error(
        `[trae-enhancer] panel-open against ${API_BASE} failed: ${error?.message || error}`,
      );
      showToast(`无法连接增强服务 (${API_BASE})，请重新通过安装的 exe 启动 Trae CN`, true);
      result = null;
    } finally {
  ...
  async function openExportDialog() {
    try {
      ...
      transferSubmit = async () => {
        ...
        downloadTransfer(result, "TRAE-CODE-CN-accounts.json");
  ...
  function openPanel() {
    ...
    refresh()
      ...
      .catch((error) => {
        // Mirror the failure instead of dropping it, so the cause is inspectable.
        console.error(`[trae-enhancer] initial panel refresh failed: ${error?.message || error}`);
        syncingCheckin = false;
      });
  }
